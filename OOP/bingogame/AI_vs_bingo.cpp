...
void shuffle(int n, int bingoSize, int** bingo)
{
    for (int i = 0; i < n; i++)
    {
...
auto Print(int** bingo, int bingoSize) -> void
{
    for (int y = 0; y < 5; y++)
    {
        for (int x = 0; x < 5; x++)
...
int main()
{
    ...
    bingocountCom = bingocountUser = 0;
    ...
    while (1)
    {
        ...
        while (checkCom == -1) {
            ...
            Count(bingocountUser, bingoSize, bingoCom);
    }
    return 0;
}